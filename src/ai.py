...
def _client():
    ...
    isn't safe under concurrent use - a shared client's underlying HTTP
    transport can get closed mid-flight by one thread while others are
    still using it ('Cannot send a request, as the client has been
    closed'), causing concurrent calls to fail and fall back to slow
    sequential retries. A thread-local client still avoids recreating one
    per call within a single thread's lifetime, so reruns on the main
    thread stay cheap too."""
    t0 = _time.time()
